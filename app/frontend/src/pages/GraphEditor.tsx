...
const GraphEditor = () => {
  ...
  const [selectedRelationship, setSelectedRelationship] = useState<Link | null>(
    null
  );
  ...
  const generateEmbeddings = async (label: string) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5001/api/generate_embedding/${label}`,
        { method: "POST" }
      );
      if (response.ok) {
        ...
        toast.error(
          `Failed to generate embeddings: ${errorData.error || "Unknown error"}`
        );
      }
      setIsLoading(false);
    } catch (error) {
  ...
  const createVectorIndex = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5001/api/vector-index/nodes",
        {
          method: "POST",
  ...
  const handleVectorSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5001/api/vector-search/nodes",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="w-150 border-r border-brain-secondary bg-secondary/30 backdrop-blur-sm p-4 z-10 overflow-y-auto">
          <Tabs defaultValue="nodeLabels">
            <TabsList className="w-full mb-4">
              <TabsTrigger value="nodeLabels" className="flex-1">
                Labels
              </TabsTrigger>
              ...
              <TabsTrigger value="vectorSearch" className="flex-1">
                Vector Search
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="nodeLabels" className="space-y-4">
              ...
              <div className="space-y-2">
                {nodeLabels.map((label) => (
                  <div
                    key={label}
                    className="flex justify-between items-center p-2 bg-secondary/60 rounded-md"
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className="w-3 h-3 rounded-full"
                        style={{
                          backgroundColor: labelColors[label] || "#9b87f5",
                        }}
                      />
              ...
              <div className="space-y-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!connectionMode) {
                      toast.info(
                        "Click on source node, then target node to create a relationship"
                      );
                      setSelectedNode(null);
                      setSelectedRelationship(null);
                    } else {
            ...
            <TabsContent value="filter" className="space-y-4">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-2">
                  <Label htmlFor="filter-type" className="self-center">
                    Filter by:
                  </Label>
            ...
            <TabsContent value="vectorSearch" className="space-y-4">
              ...
              <div className="border p-4 rounded-md space-y-2">
                ...
                <Select
                  ...
                  onValueChange={setVectorSimilarity}
                >
                  <SelectTrigger>
                  ...
                  <ul className="text-white text-sm">
                    {vectorIndexes.map((idx) => (
                      <li key={idx.indexName}>
                        {idx.indexName} - {idx.label} - {idx.dimensions} dims -{" "}
                        {idx.similarityFunction}
                      </li>
                    ))}
                  </ul>
                ) : (
          ...
          {connectionMode && sourceNode && (
            <div className="mt-4 border border-brain-secondary rounded-md p-3 bg-secondary/40 relative">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {sourceNode?.nombre ||
                  sourceNode?.name ||
                  `Node ${sourceNode?.elementId}`}
              </p>
              <p className="text-sm text-muted-foreground">
                To: Click on a target node
              </p>
            </div>
          )}
            ...
            <div className="mt-6 border-t border-brain-secondary pt-4">
              <h2 className="text-lg font-medium text-white mb-4 flex justify-between">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    handleDeleteNode(selectedNode ? selectedNode.elementId : "")
                  }
                >
        ...
        <main className="flex-1 overflow-hidden z-10">
          <GraphVisualization
            ...
            highlightedNodeIds={
              filteredNodes.length > 0
                ? filteredNodes.map((node) => node.elementId)
                : null
            }
            executeQuery={executeQuery}
          />
      ...
      >
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewRelationshipType(e.target.value.toUpperCase())
                }
              />
            </div>
          </div>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setIsCreateRelationshipDialogOpen(false)}
            >
              Cancel
            </Button>
            <Button
